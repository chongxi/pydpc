...
#     dens_ = np.exp(-(clu.distances/0.531)**2)
...
class Distances(object):
    def __init__(self, points):
        ...
        self.distances = pairwise_distances(self.points).astype('float64')
...
class Density(Distances):
    def __init__(self, points, fraction):
        ...
        # dis = squareform(self.distances, checks=False)
        self.kernel_size = _np.percentile(self.distances.ravel()[::100], 100*self.fraction)
        # self.density = _core.get_density(self.distances, self.kernel_size)
        self.density = get_density(self.distances, self.kernel_size)
